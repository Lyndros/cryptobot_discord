...
def get_balance(address, json_field, multiplier):
    ...
    #Get value from explorer
        ...
        if status_code == 200:
            ...
            try:
                #Limit to decimals
                return round(float(json.loads(req.text)[CONFIG['COIN']['json_field']])*multiplier,CONFIG['COIN']['decimals'])
            except:
                return 0
...
def mostrar_inversores():
    ...
    for mn in my_addresses:
        ...
        MN_Generated = MN_Total_Mined - MN_Total_Mined*mn['operator_fee']
...
def mostrar_coste():
    ...
    for mn in my_addresses:
        ...
        for exp in my_expenses:
            embed.description+= exp['desc'] + " {0:.{1}f}".format(exp['cost'], 2)+'\n'
